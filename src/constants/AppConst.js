import { Platform } from 'react-native'
import { colors } from '../styles/colors';
import en from './languages/en';

export class AppConst {

  static isIosDevice() {
    if (Platform.OS === 'ios') {
      return true;
    }
    return false;
  }


  static appThis = null;
  static getAppThis() {
    return this.appThis;
  }
  static setAppThis(appThis) {
    this.appThis = appThis;
  }

  static isSetDeviceToken = false;
  static getIsSetDeviceToken() {
    return this.isSetDeviceToken;
  }
  static setIsSetDeviceToken(isSetDeviceToken) {
    this.isSetDeviceToken = isSetDeviceToken;
  }

  static selectedLanguage = "en";
  static getSelectedLanguage() {
    return this.selectedLanguage;
  }

  static setSelectedLanguage(lan) {
    this.selectedLanguage = lan
  }

  static appBuild = null;
  static setAppBuild(appBuild) {
    this.appBuild = appBuild;
  }

  static deviceToken = null;
  static getDeviceToken() {
    return this.deviceToken;
  }
  static setDeviceToken(deviceToken) {
    this.deviceToken = deviceToken;
  }

  static clientId = null;
  static getClientId() {
    return this.clientId;
  }
  static setClientId(clientId) {
    this.clientId = clientId;
  }

  static accessToken = null;
  static setAccessToken(accessToken) {
    this.accessToken = accessToken;
  }

  static authToken = null;
  static getAuthToken() {
    return this.authToken;
  }
  static setAuthToken(authToken) {
    this.authToken = authToken;
  }

  static accountId = null;
  static setAccountId(id) {
    this.accountId = id;
  }


  static statusHeight = {
    top: 0,
    bottom: 0,
    watchVideoHeight: 0,
  };
  static getStatusHeight() {
    return this.statusHeight;
  }
  static setStatusHeight(statusHeight) {
    this.statusHeight = statusHeight;
  }

  static replceAll(str, find, replace) {
    var re = new RegExp(find, 'g');
    return str.replace(re, replace);
  }
  static showConsoleLog(message, ...optionalParams) {
    console.log(message, ...optionalParams);
  }

  static jsonCopy = item => JSON.parse(JSON.stringify(item))
}




const razorpayLiveKey = '';
const razorpayTestKey = '';
export const RazorpayConst = {
  key: razorpayLiveKey, //"rzp_live_DumllS2uq63eao", //"rzp_test_ErlccR6IZTfR7W",
  name: "",
  secret: ""
}

export const BaseValidation = {
  alphaNumeric: /^[a-zA-Z0-9]*$/,
  email: /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/,
  validEmail: /^\w+([\.-] ?\w +)*@\w + ([\.-] ?\w +)* (\.\w{ 2, 3 }) +$ /,
  url: /^(?:(?:https?|ftp):\/\/)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?$/,
  password: /^(?=.*\d)(?=.*[!@#$%^&*. ])(?=.*[a-z])(?=.*[A-Z]).{6,}$/,
  atleastOneAlphaOneNumeric: /^(?=.*[0-9])(?=.*[a-zA-Z])([a-zA-Z0-9]+)$/,
  alphabatic: /^[a-zA-Z]*$/,
  namealphabatic: /^[a-zA-Z ]*$/,
  numeric: /^[0-9]*$/,
  price: /^(\d*\.)?\d+$/,
  existAlphabatic: /[a-zA-Z]/,
  existOneNumber: /\d/,
  specialCharacter: /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/,
  lowerCharacter: /[a-z]/,
  uppercaseCharacter: /[A-Z]/
};


export const AsyncItemKeys = {
  onBoarding: "onBoarding",
  accessToken: "accessToken",
  userRole: "userRole"
}

export const adminRole = "reportingOperator"
export const adminBlockWiseRole = "reportingOperatorBlockWise";

export const noImage = 'data:image/png;base64,'

export const docTypeImage = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9RsiK7WSMHn6rVq5pLvEgINRyVTNKJ7drsg&usqp=CAU"

export const PriceSign = "$";

export const getDiscountWithtype = (type, discount) => {
  return `${type == "flat" ? PriceSign : ''}${discount}${type == "percentage" ? '%' : ''}`
}

export const returnPrice = (price, currency, discount = false) => {
  let sign = "\u20B9";
  // console.log(price)
  if (price == "free" || price == "Free" || price == null) {
    return sign + " " + "0"
  }
  // else {
  //   if (discount) {
  //     return ' '
  //   }
  return sign + " " + price
  // }
  return ''
}


export function getAge(dateString) {
  var today = new Date();
  var birthDate = new Date(dateString);
  var age = today.getFullYear() - birthDate.getFullYear();
  var m = today.getMonth() - birthDate.getMonth();
  if (m < 0 || (m === 0 && today.getDate() < birthDate.getDate())) {
    age--;
  }
  return age;
}





export const getStatusColor = (status) => {
  switch (status) {
    case 'pending':
      return colors.red;
    case 'completed':
      return colors.green;
    case 'confirmed':
      return colors.green;
    case 'cancelled':
      return colors.yellow;
    case 'expired':
      return colors.yellow;
    case 'refunded':
      return colors.yellow;
    default:
      return colors.primary;
  }
}



export const getLanguageText = (text) => {
  const selectedLang = AppConst.selectedLanguage;
  switch (selectedLang) {
    case "en":
      return en[text];
    default:
      return en[text];
  }
}