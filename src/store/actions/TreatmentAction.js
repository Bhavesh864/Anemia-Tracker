...
export const getPctsListByMinMaxAction = async (loader = true, search = '') => {
    try {
        const res = await GetRequest({
            url: getPctsListByMinMaxUrl + `limit=9&page=0&min=1&max=9&search=${search}`,
            loader
        });
        return res;
    } catch (error) {
