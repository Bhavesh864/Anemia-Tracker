...
import { getStudentDetail } from '../../../store/actions/TreatmentAction';
...
const StudentDetailScreen = ({ route, navigation }) => {
    ...
    //console.log('detail.hb_value', detail.hb_value);
    return (
        <View style={screenStyle}>
            ...
            {(detail && detail.hb_test_id && detail.hbTestInfo?.length > 0 && detail.hbTestInfo[0].hb_value <= 11 && (profile && profile.role !== adminRole && detail.hb_value < 11)) ?
                ((detail?.hb_treatment_expiry_date && (new Date().getTime() >= new Date(moment(detail?.hb_treatment_expiry_date))?.getTime())) || type == "test") ?
                    ...
                    !detail.hbTestInfo[0].date_of_treatment && detail.hb_value < 11 ?
                        <View style={{ width: "70%", alignSelf: "center", position: "absolute", bottom: 20 }}>
                            <Button
                                title='New Treatment'
                                onPress={() => navigate("newTreatment", { detail: { ...detail, hbTest: detail.hbTestInfo[0] } , hbTestDate: detail.hbTestInfo[0].hb_test_date})}
                            />
                        </View>
                        : null
                : null
