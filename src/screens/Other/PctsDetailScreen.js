...
const PctsDetailScreen = ({ route, navigation }) => {
    ...
    const formattedLmpDate = moment(pctsDetail?.LMP_Date, 'DD/MM/YYYY').format('YYYY-MM-DD');
    const [lmpDateDiff, setlmpDateDiff] = useState(moment(currentMoment).diff(formattedLmpDate, 'months'));
    ...
    function getDetail() {
        ...
        getPctsDetailAction(paramData.id).then(res => {
            // AppConst.showConsoleLog("---------res: Ki Detail----", res);
            // AppConst.showConsoleLog("---------res: Ki Detail----", paramData);
            if (res?.status) {
                setPctsDetail({ ...paramData, hbTestHistory: res?.data });
                setmyList(res.data);
            }
        })
        //console.log('pcts details', pctsDetail);
    }

    // AppConst.showConsoleLog("exp: ", (moment().isSameOrAfter(null) && pctsDetail?.hbTestHistory?.length > 0));
    // AppConst.showConsoleLog('expiry date',pctsDetail?.hbTestHistory[0].hb_test_date)
    // AppConst.showConsoleLog((moment().isSameOrAfter(pctsDetail?.hb_treatment_expiry_date)));
    AppConst.showConsoleLog('difference', lmpDateDiff);
    ...
    return (
        <View style={screenStyle}>
            ...
            {((moment().isSameOrAfter(pctsDetail?.hb_treatment_expiry_date) || !pctsDetail?.hb_treatment_expiry_date) && (!pctsDetail?.hbTestHistory?.length > 0) || !pctsDetail.hb_treatment_expiry_date) && myList?.length != 0 ?
                <View View style={styles.floatView}>
                    <Button
                        title='New Treatment'
                        onPress={() => {
                            navigate("newTreatment", { detail: { ...paramData, hbTest: pctsDetail?.hbTestHistory[0], isPcts: true }, lmpDateDiff, hbTestDate:pctsDetail?.hbTestHistory[0].hb_test_date });
...
const HbTestList = ({ list = [], paramData, pctsDetail }) => {
            ...
            item.date_of_treatment ? <View style={{ padding: 20 }}>
                ...
                <View >
                    <Button style={{ width: '50%' }}
                        title='New Treatment'
                        onPress={() => {
                            navigate("newTreatment", { detail: { ...paramData, hbTest: pctsDetail?.hbTestHistory[0], isPcts: true }, hbTestDate: item.hb_test_date });
                    ...
                    <View key={String(index)} style={styles.testCard}>
                        ...
                        <View style={styles.optnsView}>
                            <TouchableOpacity
                                ...
                                style={[styles.optn, !activeTab.includes(item.id) ? { borderBottomWidth: 2 } : null, { width: item.date_of_treatment ? "50%" : "50%" }]}
                            >
                                <HeadingText
                                    text='HB Test'
                                    size={16}
